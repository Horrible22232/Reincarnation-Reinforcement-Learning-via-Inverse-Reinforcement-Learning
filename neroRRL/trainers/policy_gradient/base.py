...
import neroRRL
from neroRRL.sampler.trajectory_sampler import TrajectorySampler
from neroRRL.sampler.recurrent_sampler import RecurrentSampler
from neroRRL.sampler.transformer_sampler import TransformerSampler
...
class BaseTrainer():
    ...
    def __init__(self, configs, sample_device, train_device, worker_id = 1, run_id  = "default", out_path = "./", seed = 0, compile_model = False):
        ...
        self.expert_gamma = configs["trainer"]["expert_gamma"] if "expert_gamma" in configs["trainer"] else None
        self.expert_lamda = configs["trainer"]["expert_lamda"] if "expert_lamda" in configs["trainer"] else None
        ...
        if self.recurrence is None and self.transformer is None:
            self.sampler = TrajectorySampler(self.run_id, configs, worker_id, self.vis_obs_space, self.vec_obs_space,
        ...
        # Instantiate sampler for recurrent policies
            ...
            self.sampler = RecurrentSampler(self.run_id, configs, worker_id, self.vis_obs_space, self.vec_obs_space,
                                        self.action_space_shape, self.model, self.sample_device, self.train_device)
        # Instantiate sampler for transformer policoes
        elif self.transformer is not None:
            self.sampler = TransformerSampler(self.run_id, configs, worker_id, self.vis_obs_space, self.vec_obs_space,
    ...
    def step(self, update):
        ...
        self.sampler.buffer.calc_advantages(last_value, last_expert_value, self.gamma, self.lamda, self.expert_gamma, self.expert_lamda)
    ...
    def collect_checkpoint_data(self, update):
        ...
        checkpoint_data["version"] = neroRRL.__version__
    ...
    def load_checkpoint(self, path):
        ...
        if checkpoint["version"] != neroRRL.__version__:
