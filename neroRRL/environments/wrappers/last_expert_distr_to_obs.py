...
class LastExpertDistibutionToObs(Env):
    """This wrapper adds the last expert distribution to the vector observation space of the environment."""
    ...
    def __init__(self, env):
        ...
        """
        self._env = env
        # Specify the number of actions (works only for the crafter environment)
    ...
    def reset(self, reset_params = None):
        ...
        # It's empty because the expert distribution will be added later in the sampling process
        if vec_obs is None:
            vec_obs = np.zeros(self._num_actions)
        else:
            vec_obs = np.concatenate((vec_obs, np.zeros(self._num_actions)), axis = 0)
