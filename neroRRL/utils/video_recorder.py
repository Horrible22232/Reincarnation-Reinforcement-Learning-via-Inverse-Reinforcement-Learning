...
class VideoRecorder:
    ...
    def __init__(self, video_path, frame_rate, website_name):
        ...
        self.cwd = self.cwd[:self.cwd.rfind("neroRRL") -1] # Fixed relative path
        self.website_path = self.cwd + "/result/"
        self.fourcc = cv2.VideoWriter_fourcc(*'mp4v')   # Video codec
        self.frame_rate = int(frame_rate)
        self.website_name = "result_website" if website_name is None else website_name
    ...
    def generate_website(self, trajectory_data, configs):
        ...
        expert_action_probs = torch.cat(trajectory_data["expert_probs"]).squeeze().tolist() if "expert_probs" in trajectory_data else []
        expert_actions = trajectory_data["expert_actions"] if "expert_actions" in trajectory_data else []
        ...
        expert_info = self._config_to_html(configs["expert"]) if "expert" in configs else "Not used"
        ...
        # Render the template
        with open(self.website_path + self.website_name + '_' + str(id) + '.html' , 'w') as output_file:
            output_file.write(template.render(envInfo=env_info,
                                            ...
                                            yExpertAction=str(expert_action_probs),
                                            expertAction = str(expert_actions),
                                            action=str(actions),
    ...
    def _render_environment_episode(self, trajectory_data, path, video_id):
        ...
        for i in range(len(trajectory_data["vis_obs"])):
            ...
            if "expert_rewards" in trajectory_data and i < len(trajectory_data["expert_rewards"]):
                self.draw_text_overlay(info_frame, 508, 20, round(trajectory_data["expert_rewards"][i], 3), "Expert reward per step:")
