...
from neroRRL.evaluator import Evaluator
from neroRRL.trainers.policy_gradient.ppo_shared import PPOTrainer
from neroRRL.trainers.policy_gradient.ppo_decoupled import DecoupledPPOTrainer
from neroRRL.utils.monitor import TrainingMonitor
from neroRRL.utils.utils import aggregate_episode_results, set_library_seeds
...
class Training():
    ...
    def run(self):
        ...
        for update in range(self.resume_at, self.updates):
            ...
            if episode_result:
                expert_log = ""
                if "expert_reward_mean" in episode_result:
                    expert_log = " expert_reward={:.2f} std={:.2f} expert_optimal_reward={:.2f} std={:.2f} rel={:.2f}".format(
                        episode_result["expert_reward_mean"], episode_result["expert_reward_std"], episode_result["expert_optimal_reward_mean"],
                        episode_result["expert_optimal_reward_std"], episode_result["rel_expert_optimal_reward_mean"])
                self.monitor.log((("{:4} sec={:2} env_reward={:.2f} std={:.2f} length={:.1f} std={:.2f} ") + expert_log +
