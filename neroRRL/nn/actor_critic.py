...
from neroRRL.nn.base import ActorCriticBase
from neroRRL.nn.heads import MultiDiscreteActionPolicy, ValueEstimator, AdvantageEstimator
...
class ActorCriticSharedWeights(ActorCriticBase):
    ...
    def __init__(self, config, vis_obs_space, vec_obs_shape, action_space_shape):
        ...
        if config["value_head"] == "multi":
            print("Using multi-value head")
            print(self.out_features_body + 1)
            self.critic_expert = ValueEstimator(self.out_features_body + 1, self.activ_fn)
    ...
    def forward(self, vis_obs, vec_obs, memory = None, mask = None, memory_indices = None, sequence_length = 1, expert_reward_coefficient = None):
        """Forward pass of the model
        ...
        Returns:
            {list} -- Policy: List featuring categorical distributions respectively for each policy branch
            {torch.tensor} -- Value function: Value
            {torch.tensor} -- Value function: Value of the expert reward (None if the model is not multi-value)
            {torch.tensor or tuple} -- Current memory representation or recurrent cell state (None if memory is not used)
        """
        # Forward observation encoder
        if vis_obs is not None:
        ...
        expert_value = None
        if self.critic_expert is not None:
            if expert_reward_coefficient is None:
                # print("WARNING: expert_reward_coefficient is None")
                pass
            else:
                expert_reward_coefficient = torch.tensor(expert_reward_coefficient, dtype=h.dtype).unsqueeze(1)
                h_expert = torch.cat((h, expert_reward_coefficient), 1)
                expert_value = self.critic_expert(h_expert)
        # Head: Policy branches
        pi = self.actor_policy(h)

        return pi, value, expert_value, memory, None
