...
class Buffer():
    ...
    def __init__(self, configs, visual_observation_space, vector_observation_space,
        ...
        self.use_expert = "expert" in configs
    ...
    def init_default_buffer_fields(self):
        ...
        if self.use_expert:
            ...
            self.expert_reward_coefficient = np.zeros((self.num_workers, self.worker_steps), dtype=np.float32)
            self.expert_logits = torch.zeros((self.num_workers, self.worker_steps, self.action_space_shape[0]))
            if self.configs["model"]["value_head"] == "multi":
                self.expert_values = torch.zeros((self.num_workers, self.worker_steps))
    ...
    def _calc_advantages(self, last_value, values, gamma, lamda, rewards):
        ...
        Arguments:
            ...
            values {numpy.ndarray} -- Values of the agent's states
    ...
    def calc_advantages(self, last_value, last_expert_value, gamma, lamda, expert_gamma, expert_lamda):
        ...
        Arguments:
            ...
            gamma {float} -- Discount factor
            lamda {float} -- GAE regularization parameter
            expert_gamma {float} -- Discount factor for the expert reward
            expert_lamda {float} -- GAE regularization parameter for the expert reward
        """
        ...
        if self.configs["model"]["value_head"] == "default":
            ...
            if self.use_expert:
                self.advantages = self._calc_advantages(last_value, self.values, gamma, lamda, self.env_rewards + self.expert_rewards)
            else:
                self.advantages = self._calc_advantages(last_value, self.values, gamma, lamda, self.env_rewards)
        # Check if the model has two value heads
        if self.configs["model"]["value_head"] == "multi":
            # Calculate advantages for the expert value head
            self.advantages_expert = self._calc_advantages(last_expert_value, self.expert_values, expert_gamma, expert_lamda, self.expert_rewards)
            # Calculate advantages for the default value head
            self.advantages_env = self._calc_advantages(last_value, self.values, gamma, lamda, self.env_rewards)
            # Calculate the advantages used for the policy loss to the sum of the expert and environment advantages
            self.advantages = self.advantages_expert + self.advantages_env
    ...
    def prepare_batch_dict(self):
        ...
        # Add expert data
        if self.use_expert:
            samples["expert_reward_coefficient"] = torch.tensor(self.expert_reward_coefficient)
        ...
        # Add expert data
        if self.use_expert:
            samples["expert_logits"] = self.expert_logits
            # Add expert data if multiple value heads are used
            if self.configs["model"]["value_head"] == "multi":
                samples["expert_values"] = self.expert_values
                samples["advantages_expert"] = self.advantages_expert
                samples["advantages_env"] = self.advantages_env
    ...
    def mini_batch_generator(self, num_mini_batches):
        ...
        for start in range(0, self.batch_size, mini_batch_size):
            ...
            for key, value in self.samples_flat.items():
                ...
                # Add expert data if expert reward is used
                if self.use_expert:
                    mini_batch["expert_reward_coefficient"] = value[mini_batch_indices].to(self.train_device)
            yield mini_batch
    ...
    def recurrent_mini_batch_generator(self, num_mini_batches):
        ...
        for num_sequences in num_sequences_per_batch:
            ...
            for key, value in self.samples_flat.items():
                ...
                elif key == "log_probs" or "advantages" in key or key == "values" or key == "expert_values" or key == "advantages_expert" or key == "expert_logits":
